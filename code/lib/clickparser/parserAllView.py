...
def parserAllView(file_click, id):
    ...
    for line in text.splitlines():
        ...
        if elem_class_control == True and string.find(line, '}') != -1:
            elem_class_control = False
            subgraph_ele_class(line2,
                               ele_class_dict)  # (contenutodellelementclass, dizionario delle classelement,dizionario element)
            line2 = ''
            continue
            ###########
        ...
        if string.find(line, '{') != -1 and string.find(line,
                                                        '}') != -1:  # crea il nodo che corrisponde al compound element
            compound_list = []
            element_list = load_list(line, compound_list)

            compound_line = line[string.find(line, '{') + 1:string.find(line,
                                                                        '}')]  # contiene tutta la dichiarazione del compound element
    ...
    for comp in compound_element.items():
        words3 = []
        explicit_compound_decl(comp[1]['compound'], element, comp[1]['name'] + '.', comp[1]['name'], words,
                               element_renamed)
    ...
    connection_element_class_output_closer(connection_list, fluxOutput,
                                           clean_ele_class_connections)  # gestisce gli output
