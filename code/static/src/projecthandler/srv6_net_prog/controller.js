...
dreamer.Srv6_net_progController = (function(global) {
    'use strict';

    var DEBUG = true;

    Srv6_net_progController.prototype.constructor = Srv6_net_progController;

    /**
     * Constructor
     */
    function Srv6_net_progController() {


    }


    Srv6_net_progController.prototype.addNode = function(graph_editor, node, success, error) {
        log('addNode');
        var data_to_send = {
    ...
    Srv6_net_progController.prototype.addLink = function(graph_editor, link, success, error) {
        log('addLink');
        var data_to_send = {
    ...
    Srv6_net_progController.prototype.removeNode = function(graph_editor, node, success, error) {
        log('removeNode');
        var data_to_send = {
            'group_id': node.info.group[0],
            'element_id': node.id,
    ...
    function log(text) {
        if (DEBUG)
            console.log("::Srv6_net_progController::", text);
    }

    return Srv6_net_progController;
}(this));

if (typeof module === 'object') {
    module.exports = dreamer.Srv6_net_progController;
}